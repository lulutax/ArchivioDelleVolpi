...
<title>L'Archivio delle Volpi</title>
	...
	<link rel="stylesheet" href="../css/serietv.css" type="text/css" />
	<script src="../js/eventi.js"></script>
	<script src="../js/apiSerie.js"></script>
</head>
<body>
<div class="text-center">
  <img src="images/background.png" class="col-lg-4 col-md-8 col-sm-12">
	...
	<h2 id="id_titolo">${serie.titolo}</h2>
	<section class="row col-lg-8 col-md-8 col-sm-8">
		<div>
			<img src="images/${serie.pathImage}" alt="${serie.titolo}" width="200" height="300" id="immagine">
		</div>
		<div id="descrizione">
			<h6><b>Genere:</b> ${serie.genere}</h6>
			<h6 id="line"><b>Ideatore:</b> <h6 id="regista">${serie.regista}</h6></h6>
			<h6><b>Anno:</b> ${serie.annoUscita}</h6>
			<div class="dropdown dropright">
				<select id="stagione" onChange="mostraInfo(this);" type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown" required>
					<option selected>Seleziona Stagione</option>
	      			<c:forEach var="st" items="${stagioni}">
			        	<option value="${st}">Stagione ${st}</option>
			     	</c:forEach>
	    		</select>
		    </div>
		    <h6 id="episodi"><b>Numero episodi:</b> </h6>
			<h6 id="durata"><b>Durata episodio:</b> </h6>
			<h6><b>Disponibile su:</b> ${serie.piattaforme}</h6>
			<c:choose>
		    	<c:when test="${id=='admin'}">
				  	<h6><a href="rimuoviDalCatalogoSerie?titolo=${serie.titolo}" id="aggiungi">Rimuovi Serie</a></h6>
			  	</c:when>
			  	<c:otherwise>
	...
	<h4><b>Trama</b></h4>
	<p id="trama">${serie.descrizione}</p>
		      			...
		      			<c:if test="${u.email==rec.utente}">
			      			<h6><img src="${u.profilo}" class="rounded-circle" width="40" height="40">&ensp;<b id="nomeRec">${rec.utente}:</b> ${rec.testo}&ensp;
			      				<c:if test="${u.email==id}">
			      			 		<a href="rimuoviReceSerie?utente=${rec.utente}&testo=${rec.testo}&titolo=${serie.titolo}"><i class="fa fa-trash" aria-hidden="true"></i></a>
			      			 	</c:if>
		      			 	</h6>
		      			</c:if>
	      			</c:forEach>
